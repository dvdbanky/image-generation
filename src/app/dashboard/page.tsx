...
export default function Dashboard() {
  ...
  useEffect(() => {
    const createUserProfile = async () => {
      if (isSignedIn && user && !profileCreated) {
        try {
    ...
    createUserProfile();
  }, [isSignedIn, user, profileCreated]);
    ...
    <div className="relative min-h-screen bg-black">
      ...
      <div className="fixed top-4 right-4 z-20">
        <div className="flex items-center gap-3">
          <span className="text-sm" style={{color: '#f8fafc'}}>{user?.firstName || user?.emailAddresses[0].emailAddress}</span>
